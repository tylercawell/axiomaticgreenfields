...
class BulkSMS extends Model implements Auditable
{
     ...
     */
    private static $url = "http://bulksms.2way.co.za:5567/eapi/submission/send_sms/2/2.0";

    static function Send($msisdn, $seven_bit_msg, $uID = null) {
        /*
         * Your phone number, including country code, i.e. +44123123123 in this case:
         */
        //$msisdn = '44123123123';
        //if(API_MODE == API_MODE_PRODUCTION) {
            //$msisdn = "+27842525258";
        //}
        
        $msisdn = stripos($msisdn, "0") === 0 ? "+27".substr($msisdn, 1) : $msisdn;
         ...
         */
        $post_body = BulkSMS::seven_bit_sms(BulkSMS::$username, BulkSMS::$password, $seven_bit_msg, $msisdn, $uID);
        $result = BulkSMS::send_message($post_body, BulkSMS::$url);
        $response = BulkSMS::formatted_server_response($result);
        //SystemLogController::log("BULK_SMS", "SEND", 0, 0, json_encode($response), "");
        return $response;
    ...
    static function formatted_server_response($result) {
        $this_result = array();
        $this_result['success'] = $result['success'];

        if ($result['success']) {
            $this_result['message'] = "Success: batch ID " . $result['api_batch_id'] . "API message: " . $result['api_message'] . "\nFull details " . $result['details'];
        } else {
            $this_result['message'] = "Fatal error: HTTP status " . $result['http_status_code'] . ", API status " . $result['api_status_code'] . " API message " . $result['api_message'] . " full details " . $result['details'];
            
            if ($result['transient_error']) {
                $this_result['message'] .= "This is a transient error - you should retry it in a production environment";
            }
        }
        return $this_result;
    }

    static function send_message($post_body, $url) {
        /*
    ...
    static function seven_bit_sms($username, $password, $message, $msisdn, $uID = null) {
        $post_fields = array(
            'username' => $username,
            'password' => $password,
            'message' => BulkSMS::character_resolve($message),
            'msisdn' => $msisdn,
            'allow_concat_text_sms' => 1, # Change to 1 to enable long messages
            'concat_text_sms_max_parts' => 2
        );

        return BulkSMS::make_post_body($post_fields, $uID);
    }

    static function unicode_sms($username, $password, $message, $msisdn, $uID = null) {
        $post_fields = array(
            'username' => $username,
            'password' => $password,
            'message' => BulkSMS::string_to_utf16_hex($message),
            'msisdn' => $msisdn,
            'dca' => '16bit'
        );
    ...
    static function eight_bit_sms($username, $password, $message, $msisdn, $uID = null) {
        $post_fields = array(
            'username' => $username,
            'password' => $password,
            'message' => $message,
    ...
    static function make_post_body($post_fields, $uID = null) {
        //$stop_dup_id = BulkSMS::make_stop_dup_id();
        if ($uID) {
            $post_fields['stop_dup_id'] = $uID;//BulkSMS::make_stop_dup_id();
        }
        $post_body = '';
    ...
    static function character_resolve($body) {
        $special_chrs = array(
        ...
        for ($i = 0; $i < mb_strlen($body, 'UTF-8'); $i++) {
            ...
            if (isset($special_chrs[$c])) {
                $ret_msg .= chr(hexdec($special_chrs[$c]));
            } else {
                $ret_msg .= $c;
            }
        }
    ...
    static function string_to_utf16_hex($string) {
        return bin2hex(mb_convert_encoding($string, "UTF-16", "UTF-8"));
    }

    static function xml_to_wbxml($msg_body) {
